...
운영 배포, 아키텍처 정합성, 부하테스트 결과, 확장 판단 기준을 한 위치에서 찾게 해서 장애 예방과 구조 개선 우선순위 판단을 빠르게 하는 것이 목적입니다.

## 비목적

이 문서는 Terraform 전체 리소스 명세나 모든 AWS 운영 절차를 반복 설명하지 않으며, 실제 리소스 변경은 각 배포/인프라 절차 문서를 기준으로 수행합니다.
