...
4. Cloudflare 운영 DNS를 Terraform에서 추적하려면 `cloudflare_dns/`를 별도로 적용합니다.
...
- `cloudflare_dns/`는 `timelink.cloud`의 Cloudflare DNS 레코드를 Terraform 상태에서 추적하는 선택 스택입니다.
- 기존 Cloudflare 레코드는 첫 apply 전에 import해야 합니다.
- `CLOUDFLARE_API_TOKEN`은 환경변수 또는 CI secret으로만 주입하고 Git/SSM에 저장하지 않습니다.
