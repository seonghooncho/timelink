...
import { storageApi, UploadFileAsset } from '../../services/api';
...
export function GroupFormScreen({ navigation }: Props) {
  ...
  const [image, setImage] = useState<UploadFileAsset | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const imageUrl = image ? (await storageApi.uploadGroupImage(image)).url : undefined;
      ...
      Alert.alert('생성 실패', '그룹 생성 중 오류가 발생했습니다.');
    }
  };
      ...
      <View style={styles.content}>
        <SectionCard>
          ...
          <View style={styles.imageRow}>
            ...
            <Text style={styles.imageHint}>그룹을 대표하는 사진을 추가하세요. 5MB 이하 이미지 권장.</Text>
          </View>
        </SectionCard>
