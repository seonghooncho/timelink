...
export function formatDateTimeDuration(startIso: string, duration: number, fallbackEndIso?: string) {
  const start = new Date(startIso);
  const end = fallbackEndIso ? new Date(fallbackEndIso) : new Date(start);
  if (!fallbackEndIso) {
