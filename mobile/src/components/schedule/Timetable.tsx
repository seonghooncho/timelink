...
export function Timetable({ schedules, startDate, days, onSchedulePress, onPrev, onNext }: TimetableProps) {
  ...
  useEffect(() => {
    ...
    const includesToday = Array.from({ length: days }, (_, index) => {
      const date = new Date(startDate);
      date.setDate(date.getDate() + index);
      return toIsoDate(date);
    }).includes(todayKey);
    const currentHour = new Date().getHours();
    const visibleHour = includesToday
      ? Math.max(timetable.hourStart, currentHour - 2)
      : timetable.defaultVisibleHour;
    scrollRef.current?.scrollTo({ y: visibleHour * timetable.hourHeight, animated: false });
  }, [days, startDate]);
